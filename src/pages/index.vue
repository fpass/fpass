<template>
  <div class="wrap">
    <div class="mu-appbar mu-paper-1">
      <div class="mu-appbar-title">
        <span>FINAL PASS</span></div>
      <div class="right">
        <a href="https://github.com/fpass" target="_blank" class="github">
          <svg height="32" version="1.1" viewBox="0 0 16 16" width="32">
            <path fill-rule="evenodd" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"></path>
          </svg>
        </a>
      </div>
    </div>
    <mu-card class="form">
      <div class="field">
        <mu-select-field v-model="ruleIndex" label="DOMAIN">
          <mu-menu-item v-for="(rule, index) in rules" :key="index" :value="index" :title="rule.domain + ' - ' + rule.name" />
        </mu-select-field>
      </div>
      <div class="field">
        <mu-text-field v-model="password" label="PASSWORD" type="password" />
      </div>
      <div class="field">
        <mu-raised-button class="js-ok" label="OK" primary @click="clickOk" />
      </div>
    </mu-card>
    <mu-dialog :open="!!dialog.content" :title="dialog.title">
      {{dialog.content}}
      <mu-flat-button label="OK" slot="actions" primary @click="dialog.content = ''" />
    </mu-dialog>
  </div>
</template>

<script>
  import Clipboard from 'clipboard'
  import rules from '../services/rules'
  import encrypt from '../services/encrypt'

  export default {
    asyncData () {
      return {
        rules,
        ruleIndex: '',
        password: '',
        dialog: {
          title: '',
          content: ''
        }
      }
    },
    computed: {
      encrypt () {
        const rule = this.rules[this.ruleIndex]
        if (!rule) {
          return ''
        }
        return encrypt({...rule, salt: this.password})
      }
    },
    mounted () {
      this.initClipboard()
    },
    methods: {
      initClipboard () {
        new Clipboard(this.$el.querySelector('.js-ok'), {
          text: () => this.encrypt
        })
      },
      clickOk () {
        if (isNaN(this.ruleIndex) || !this.password) {
          this.dialog = {
            title: 'ERROR',
            content: 'All form items are required'
          }
          return
        }
        this.dialog = {
          title: 'The password has been copied to the clipboard',
          content: this.encrypt
        }
      }
    }
  }
</script>

<style lang="less">
  #__nuxt {
    display: flex;
    align-items: center;
    justify-content: center;
    user-select: none;
    background: #f3f3f3;
  }

  .mu-dialog {
    max-width: 520px;
  }
</style>

<style lang="less" scoped>
  .wrap {
    width: 100%;
    padding: 0 15px;
    max-width: 750px;
    min-width: 320px;
  }

  .github {
    color: #fff;
    margin: 5px 5px 0 0;

    svg {
      fill: currentColor;
    }
  }

  .form {
    padding: 15px 0;
  }

  .field {
    padding-top: 15px;
    display: flex;
    justify-content: center;

    &:last-child {
      padding-bottom: 15px;
    }
  }
</style>
